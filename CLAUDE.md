...
| [DONE] | [S] | 011 | **Pipeline matrix: Date column + sortable by any field** | User request. Added a `Date` column (relative age, e.g. "5m ago"), default sort by Date descending, `s` to cycle sort field (Date/Project/Status/Ref/SHA/Author/Duration), `S` to reverse direction without changing field. Sort is applied to `pipelines` itself (not just row rendering) so cursor-index-based lookups (`HighlightedPipeline`, bulk actions) stay correct. |
